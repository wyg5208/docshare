...
#### 导航栏全局常驻修复 🧭
- ✅ 修复 `/browse`、`/browse/[slug]`、`/search`、`/tags`、`/tags/[slug]` 共 5 个公共页面缺失顶部导航栏的问题
- ✅ 根因：这些页面不属于任何含 Header 的路由组（`(public)`/`(dashboard)`/`(admin)`），仅继承不含 Header 的根 layout
- ✅ 修复方式：在 5 个受影响页面内显式包裹 `<Header />` 和 `<Footer />`，根节点改为 Fragment + `<main>` 三段式结构
- ✅ 消除"登录后导航栏消失"的用户错觉（实际与登录态无关，是路径偶然性造成）

### 技术改进
- 📝 在迭代文档中沉淀 Next.js App Router 路由组布局陷阱的诊断思路
- 🛣️ 留下后续将 `browse`/`search`/`tags` 整体迁入 `(public)` 路由组的重构空间

> 详见：[docs/v1.1.1_2026-06-24_导航栏全局常驻修复.md](docs/v1.1.1_2026-06-24_导航栏全局常驻修复.md)
