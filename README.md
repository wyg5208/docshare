...
#### 首页 Hero 文案 Admin 可编辑 📝
- ✅ 新增 `site_settings` 键值表，存储站点级可编辑文案（公开读、admin 写）
- ✅ 新增 Admin 页面 `/admin/site-settings`，含实时预览 + 保存 + 一键还原默认
- ✅ 首页从 DB 读取 Hero 文案，降级默认值保护（迁移未执行时不空白）
- ✅ 侧边栏新增 `Site Content` 入口

### UI 英文化

#### Settings 页文件上传控件英文化 🌐
- ✅ 修复 `/settings` Profile Picture 区域"选择文件 / 未选择任何文件"中文问题
- ✅ 隐藏原生 `<input type="file">`，自定义英文按钮 `Choose File` + `No file chosen`
- ✅ 全局扫描 `src/` 确认无其它中文 UI 文案遗漏

> 详见：[docs/v1.2.0_2026-06-24_首页文案可编辑与设置页英文化.md](docs/v1.2.0_2026-06-24_首页文案可编辑与设置页英文化.md)
