...
- ✅ 修复登出后 Next.js Router Cache 未刷新导致导航异常

### 功能增强

#### 多 Tab 状态同步 🔄
- ✅ 多浏览器 Tab 登出状态自动同步（基于 Supabase BroadcastChannel）
- ✅ 其它 Tab 登出时自动刷新 Router Cache，受保护页面正确重定向

### 技术改进
- 📝 在 `signOut` 和 `onAuthStateChange` 中补充 `router.refresh()` 调用

> 详见：[docs/v1.8.1_2026-06-25_登出后按钮失效与多Tab同步修复.md](docs/v1.8.1_2026-06-25_登出后按钮失效与多Tab同步修复.md)
